"""
Bangs processing utilities for hair mask manipulation.
Includes functions for isolating bangs regions and extending them.
"""
...
def isolate_bangs_region(mask, landmarks=None):
    """
    ...
    if landmarks is not None and len(landmarks) >= 27:
        ...
        top_percent = 0.5  # Take top 50% of the mask
        ...
        # Default to 45% when we don't have landmarks
        top_percent = 0.45
    ...
    if landmarks is not None and len(landmarks) >= 27:
        ...
        try:
            ...
            eyebrow_top = max(0, int(min(eyebrow_ys)) - 20)  # Increased padding above eyebrows
                    ...
                    try:
                        ...
                        for x in range(width):
                            if x >= 0 and x < width:
                                ...
                                curve_y = int(f(x)) - 25  # pixels above eyebrows
    ...
    gradient_height = int(bangs_bottom * 0.5)  # Increased from 40% to 50% of bangs has gradient
    ...
    for y in range(gradient_start, min(top_y + bangs_bottom, height)):
        ...
        fade_ratio = fade_ratio ** 0.6  # Changed to 0.6 for more gradual falloff
...
def extend_bangs_area(mask, extend_pixels=40, forehead_ratio=0.3, min_opacity=0.7, source_landmarks=None):
    """
    ...
    print(f"DEBUG: Start of extend_bangs_area with extend_pixels={extend_pixels}, min_opacity={min_opacity}")
    ...
    # Ensure a minimum extension of 10 pixels, but never less than the requested amount
    extend_pixels = max(10, extend_pixels)
    print(f"Final extension amount: {extend_pixels} pixels")
    
    # Calculate gradient falloff power based on extension amount
    if extend_pixels >= 70:
        falloff_power = 0.2  # Very gentle falloff for very large extensions
    elif extend_pixels >= 50:
        falloff_power = 0.3  # Gentle falloff
    elif extend_pixels >= 30:
        falloff_power = 0.4  # Medium falloff
    else:
        falloff_power = 0.5   # Steeper falloff
        
    print(f"Using falloff power: {falloff_power} for extension amount: {extend_pixels}")
    
    # For larger extensions, also allow a small upward extension to better blend with hair
    allow_upward = (extend_pixels >= 40)
    upward_pixels = min(15, extend_pixels // 8) if allow_upward else 0
    if upward_pixels > 0:
    ...
    mask_coverage = 0  # Track how many columns have mask content
    ...
    # If less than 5% of the width has mask content, create a default region
    if mask_coverage < (width * 0.05):
        print("WARNING: Mask has very limited coverage. Creating a default extension region.")
        # Create a default extension region in the middle 60% of the width
        start_x = int(width * 0.2)
        end_x = int(width * 0.8)
        for x in range(start_x, end_x):
            # Set a default value in the middle of the top third of the image
            bottom_boundary[x] = max(bottom_boundary[x], int(height * 0.3))
    ...
    for x in range(width):
        ...
        # If the bottom_y is valid and within the mask bounds
        if 0 <= bottom_y < height and 0 <= x < width:
            original_value = original_mask[bottom_y, x]
        else:
            # Use a default value if out of bounds
            original_value = 128  # Middle gray value
        
        # MODIFIED: Lower the threshold to extend more areas, especially for thin hair
        if original_value < 10:  # Reduced threshold
            original_value = 128  # Use a reasonable default value
        ...
        # IMPORTANT: Ensure we're extending at least a few pixels even in edge cases
        if extension_amount <= 0:
            # Try to find a reasonable extension amount if we can't extend downward
            extension_amount = min(10, height - 1)
            if extension_amount <= 0:
                continue  # Skip if we really can't extend
        ...
        for y_offset in range(1, extension_amount + 1):
            ...
            # MODIFIED: Improved falloff calculation with better minimum visibility
            # Calculate falloff ratio (1.0 at top, decreasing downward)
            # Ensure that even at maximum extension, we maintain at least 40% opacity
            falloff = max(0.4, 1.0 - (y_offset / extension_amount) ** falloff_power)
        ...
        if upward_pixels > 0:
            ...
            if 0 <= top_y < height and 0 <= x < width:
                top_value = original_mask[top_y, x]
            else:
                # Use a default value if out of bounds
                top_value = 128  # Middle gray value
            
            # MODIFIED: Lower threshold for upward extension
            if top_value < 10:
                top_value = 100  # Use a reasonable default value
            ...
            for y_offset in range(1, upward_amount + 1):
                ...
                # More aggressive falloff for upward extension but still ensure visibility
                falloff = max(0.3, 1.0 - (y_offset / upward_amount) ** 0.8)
                
                # Reduce opacity more for upward extension but maintain visibility
                opacity = int(top_value * falloff * 0.8)
    ...
    print(f"Extended {pixels_extended} pixels from the original mask")
    
    # MODIFIED: For very small pixel extensions, amplify the visibility
    if pixels_extended < (width * extend_pixels * 0.1):  # If we extended fewer pixels than expected
        print("Limited extension detected, amplifying extension visibility")
        # Find the extended area (pixels not in original mask but in extended mask)
        extended_area = (extended_mask > 0) & (original_mask == 0)
        # Increase opacity in this area
        extended_mask[extended_area] = np.minimum(extended_mask[extended_area] * 1.5, 255).astype(np.uint8)
    
    # Smooth the result slightly for natural appearance
    # Use a larger kernel for greater extensions to create a more natural transition
    if extend_pixels >= 50:
        kernel_size = 5
    else:
        kernel_size = 3
        
    return cv2.GaussianBlur(extended_mask, (kernel_size, kernel_size), 0)

def create_face_protection_mask(mask, landmarks, protection_strength):
    """
...
def detect_hair_parting(mask, landmarks=None, enhanced_mask=None):
    """
    ...
    Args:
        ...
        enhanced_mask: Pre-allocated enhanced mask (will create a copy if None)
        
    Returns:
        tuple: (parting_line, enhanced_mask)
    """
    # Create copy of mask if enhanced_mask not provided
